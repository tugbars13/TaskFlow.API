...
export const getPriorityBorderClass = (priority) =>
  PRIORITY_BORDER[String(priority ?? "").toLowerCase()] ?? PRIORITY_BORDER.medium;

export default function TaskCard({ task, draggable = false, onDragStart, onClick }) {
  return (
    <div
      draggable={draggable}
    ...
    >
      ...
      <p className="text-xs text-on-surface-variant/80 truncate">
        {task.description || "Finalize feature implementation and design token alignment."}
      </p>
