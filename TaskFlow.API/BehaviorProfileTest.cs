    ...
    public static class BehaviorProfileTest
    {
        public static async Task RunTests(IServiceProvider sp)
        {
            ...
            try
            {
                ...
                var t1 = new TaskItem { Title = "T1", UserId = user.Id, CategoryId = 1002, Priority = TaskPriority.High, Status = TaskFlow.API.Models.TaskStatus.Completed, IsCompleted = true, CreatedDate = DateTime.UtcNow.AddDays(-10), CompletedDate = DateTime.UtcNow.AddDays(-8), DueDate = DateTime.UtcNow.AddDays(-9), IsArchived = true, IsDeleted = false };
                
                // 2. IsArchived = false, IsDeleted = true => Should NOT be INCLUDED
                var t2 = new TaskItem { Title = "T2", UserId = user.Id, CategoryId = 1002, Priority = TaskPriority.Medium, Status = TaskFlow.API.Models.TaskStatus.Completed, IsCompleted = true, CreatedDate = DateTime.UtcNow.AddDays(-5), CompletedDate = DateTime.UtcNow.AddDays(-4), IsArchived = false, IsDeleted = true };
                
                // 3. Late task -> creates high risk
                var t3 = new TaskItem { Title = "T3", UserId = user.Id, CategoryId = 1002, Priority = TaskPriority.Medium, Status = TaskFlow.API.Models.TaskStatus.Completed, IsCompleted = true, CreatedDate = DateTime.UtcNow.AddDays(-5), CompletedDate = DateTime.UtcNow.AddDays(-2), DueDate = DateTime.UtcNow.AddDays(-4), IsArchived = false, IsDeleted = false };
                ...
                var devBehavior = profile.CategoryBehaviors.FirstOrDefault(c => c.CategoryId == 2);
